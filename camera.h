...
public:
    ...
    double vfov = 90.0; // vertical view angle in degrees
    ...
    void render (const hittable& world){
        ...
        std::cout << "P3\n" << image_width << ' ' << static_cast<int>(image_width/aspect_ratio) << "\n255\n";
...
private:
    ...
    void initialize(){
        ...
        // calc upper left pixel location
        auto viewport_upper_left = center - (focus_dist * w) - viewport_u/2 - viewport_v/2;
        pixel00_loc = viewport_upper_left + 0.5 * (pixel_delta_u + pixel_delta_v);

        // calc defocus disk vectors
        auto defocus_disk_radius = focus_dist * tan(degrees_to_radians(defocus_angle / 2));
